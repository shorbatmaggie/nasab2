...
function _3(md) { return (md``) } // HIDDEN: no "## Code" header shown
...
function extractAncestry(targetId, fullData) {
  ...
  const assignLevel = (node, level = 0) => {
    if (!node || genMap.has(node.id)) return;
...
function _dropdown(fullData) {
  ...
  const flat = fullData.flat();
  const candidates = flat.filter(d => d.parents && d.parents.length > 0);
  ...
  select.onchange = () => {
    ...
    const subgraph = extractAncestry(selected, fullData);
  ...
  return (data, options = {}) => {
    ...
    <svg width="${tangleLayout.layout.width}" height="${tangleLayout.layout.height}" style="background-color: ${background_color}">
      ...
      ${tangleLayout.nodes.map(n => `
        <path class="selectable node" stroke="black" stroke-width="8"
              d="M${n.x} ${n.y - n.height / 2} L${n.x} ${n.y + n.height / 2}"/>
        <path class="node" stroke="white" stroke-width="4"
              d="M${n.x} ${n.y - n.height / 2} L${n.x} ${n.y + n.height / 2}"/>

        <text class="selectable" x="${n.x + 4}" y="${n.y - n.height / 2 - 4}" stroke="${background_color}" stroke-width="2">
          ${n.id}
          <title style="font-size: 20px;">Author: ${n.author}, (d. ${n.death} AH)</title>
        </text>
...
function _9(md) { return (md``) } // HIDDEN: No Dependencies heading
